...
function buildVehiclesDisplay($vehicles)
{
    ...
    foreach ($vehicles as $vehicle) {
        $dv .= '<li>';
        $dv .= "<a href='/phpmotors/vehicles/?action=vehicle&invId=" . urlencode($vehicle['invId']) . "' title='View our $vehicle[invMake] $vehicle[invModel]'>" . "<img src='$vehicle[imgPath]' alt='Image of $vehicle[invMake] $vehicle[invModel] on phpmotors.com'></a>";
        $dv .= '<hr>';
...
function buildVehicleDetailsDisplay($vehicle, $tns)
{
    //$dv = "<div><img alt='" . $vehicle['invMake'] . " " . $vehicle['invModel'] . "'" . " src='" . $vehicle['imgPath'] . "'>  <p class='margin0'>Price: " . "<span class='currSign'>" . number_format($vehicle['invPrice']) . "</span>" . "</p></div>";
    //$dv .= "<div>
    ...
    $dv = "<h2 class='tn-heading'>Thumbnails:</h2>";
    $dv .= "<ul class='tn-container'>";
    
    foreach ($tns as $tn) {        
        $dv .= "<li>";
        $dv .= "<img alt='".$vehicle['invMake'] . " " . $vehicle['invModel']. " by PHPmotors img' src='". $tn['imgPath'] ."'>";
        $dv .= "</li>";
    }
    $dv .= "</ul>";
    $dv .= "<div><img alt='" . $vehicle['invMake'] . " " . $vehicle['invModel'] . " by PHPmotors tn'" . " src='" . $vehicle['imgPath'] . "'>  <p class='vehicle-price'>Price: " . "<span class='currSign'>" . number_format($vehicle['invPrice']) . "</span>" . "</p></div>";
    ...
    $dv .= "<div>
    <h2>" . $vehicle['invMake'] . " " . $vehicle['invModel'] . " Details:" . "</h2>";
    $dv .= "<div class='vehicleDescription marginT10px'><p>" . $vehicle['invDescription'] . "</p>";
    $dv .= "<p class='marginT20px'>Color: " . $vehicle['invColor'] . "</p>";
    $dv .= "<p>In stock: " . $vehicle['invStock'] . "</p></div>";
    $dv .= "</div>";
